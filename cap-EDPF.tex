...
\nabla^2\Psi=g(\vec{x}),
\end{equation}
donde $g(\vec{x})$ es una función ``fuente'' conocida. Esta EDP es \textit{inhomogénea}, y por lo tanto sus soluciones generales pueden escribirse como $\Psi=\Psi_\text{h}+\Psi_\text{p}$, donde $\Psi_\text{h}$ es solución de la ecuación homogenea correspondiente (en este caso, la ec. de Laplace), y $\Psi_\text{p}$ una \textbf{solución particular} de la ec. de Poisson.
