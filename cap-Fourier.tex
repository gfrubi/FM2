...
\end{equation}
donde los coeficientes $c_n$ est'an dados por
\begin{equation}
...
\end{equation}
Los coeficientes de la series (\ref{sf1}) y (\ref{sf2}) est'an relacionados por
\begin{equation}c_n = 
\begin{cases}
...
\begin{itemize}
\item Una sucesi'on $S_n(x)$ de funciones definidas en el intervalo $x\in[-\pi,\pi]$ \textit{converge en media} a una funci'on $f(x)$ si
\begin{equation}
\lim_{n\to\infty}\int_{-\pi}^{\pi}\left[f(x)-S_n(x)\right]^2dx=0.
\end{equation}

``La sucesi'on $S_n$ converge a $f(x)$ excepto en un conjunto de medida cero''
\item Una sucesi'on $S_n(x)$ de funciones \textit{converge uniformemente} a una funci'on $f(x)$ si para todo $\epsilon>0$ existe un $N>0$ tal que
\begin{equation}
\left|f(x)-S_n(x)\right|<\epsilon, \qquad \forall n>N,
\end{equation}
para cada $x$ ($N$ independiente de $x$).
\end{itemize}
Teorema \cite{Butkov}: Si $f(x)$ es una funci'on es ``muy suave por tramos'' (es decir, si la funci'on, su primera y segunda derivadas son continuas por tramos) en el intervalo $(-\pi,\pi)$ entonces su serie de Fourier converge a
\begin{equation}
\frac{1}{2}\left[f(x-0)+f(x+0)\right], \qquad x\in (-\pi,\pi),
\end{equation}
\begin{equation}
\frac{1}{2}\left[f(-\pi+0)+f(\pi-0)\right], \qquad x=\pm\pi .
\end{equation}
La convergencia es \textit{uniforme} en cada subintervalo cerrado donde $f(x)$ es continua.

Teorema \cite{Butkov}: Si una funci'on definida en un intervalo cerrado $[a,b]$ satisface las condiciones de Dirichlet (es decir, si a) $f(x)$ es continua por tramos, y b) el intervalo $(a,b)$ puede ser dividido en un n'umero finito de subinteralos donde $f(x)$ es monotona) entonces tambi'en se satisfacen las propiedades de convergencia del Teorema 1.

Teorema \cite{Butkov}: Si la funci'on $f(x)$ es \textit{cuadrado-integrale} en $(-\pi,\pi)$ ($f\in {\cal L}^2(-\pi,\pi)$, es decir, si $\int_{-\pi}^\pi|f(x)|^2dx$ es finito) entonces su serie de Fourier converge en media a $f(x)$.
