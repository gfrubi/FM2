...
Consideraremos aquí la ecuación diferencial de Bessel con argumento complejo, $z\in\mathbb{C}$, ya que sus soluciones suminitrarán funciones que satisfacen la E.D.O. de Bessel con variables reales y la E.D.O. modificada de Bessel. Finalmente, buscaremos soluciones de la ecuación con un coeficiente $\nu^ 2$ real y positivo, pero no necesariamente entero.
...
\eqref{EDOBessel} que $y''(0)$ (y las derivadas superiores) será en general divergente, aún en el caso en que se elija $y'(0)$, debido al término $({\nu^2}/{z^2})y(z)$. 

Afortunadamente, sí es posible encontrar soluciones de la forma\footnote{Una forma alternativa de verificar que este es el caso es realizar el cambio de variable $y(z):=z^\alpha f(z)$ y ajustar la constante $\alpha$ de modo que la E.D.O. resultante para la función $f(z)$ sí pueda expresarse como una serie de potencias de la forma $f(z) = \sum_{n=0}^\infty a_n z^n$.}
...
\end{align}
Aquí hemos introducido la función Gamma, ver apéndice \ref{app-Gamma}. Por lo tanto, para $\alpha=\nu$ hemos encontrado una solución de la forma
\begin{equation}
...
Por lo tanto, la solución general de la E.D.O. de Bessel con $\nu^2=1/4$ es de la forma
\begin{equation}
